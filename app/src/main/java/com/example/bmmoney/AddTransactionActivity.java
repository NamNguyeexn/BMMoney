...
public class AddTransactionActivity extends AppCompatActivity {
    private EditText edtTitle, edtAmount, edtNote;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        prefs = getSharedPreferences("transaction_draft", MODE_PRIVATE);
        ...
        findViewById(R.id.fabCenterAdd).setOnClickListener(v -> saveTransaction());
    }

    private void saveTransaction() {
        String title = edtTitle.getText().toString().trim();
        String amountText = edtAmount.getText().toString().trim();
        if (title.isEmpty()) { edtTitle.setError("Vui lòng nhập tên giao dịch"); return; }
        if (amountText.isEmpty()) { edtAmount.setError("Vui lòng nhập số tiền"); return; }

        double amount;
        try { amount = Double.parseDouble(amountText); }
        catch (Exception e) { edtAmount.setError("Số tiền không hợp lệ"); return; }
        if (amount <= 0) { edtAmount.setError("Số tiền phải lớn hơn 0"); return; }

        String type = radioIncome.isChecked() ? "INCOME" : "EXPENSE";
        TransactionEntity transaction = new TransactionEntity(title, amount, type, spinnerCategory.getSelectedItem().toString(), edtNote.getText().toString().trim(), System.currentTimeMillis());
        long newId = database.transactionDao().insert(transaction);
        transaction.setId((int) newId);
        new FirebaseSyncManager(this).uploadTransaction(transaction);
    ...
    private void restoreDraft() {
        edtTitle.setText(prefs.getString("title", ""));
        edtAmount.setText(prefs.getString("amount", ""));
        edtNote.setText(prefs.getString("note", ""));
        radioIncome.setChecked(prefs.getBoolean("income", false));
    }
    ...
    private void setupDraftCache() {
        ...
        radioIncome.setOnCheckedChangeListener((buttonView, isChecked) -> saveDraft());
    }

    private void saveDraft() {
        prefs.edit()
                .putString("title", edtTitle.getText().toString())
                .putString("amount", edtAmount.getText().toString())
                .putString("note", edtNote.getText().toString())
                .putBoolean("income", radioIncome.isChecked())
                .apply();
    }
